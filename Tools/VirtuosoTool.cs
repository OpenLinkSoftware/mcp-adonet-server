...
public sealed class VirtuosoTools
{
    ...
    // Read our url connection string from the environment
    private static readonly string? DefaultConnStr = Environment.GetEnvironmentVariable("ADO_URL") 
        ?? "DSN=VOS;UID=demo;PWD=demo;";
    private static readonly int DefaultMaxLongData = int.TryParse(
        Environment.GetEnvironmentVariable("MAX_LONG_DATA"), out var mld) ? mld : 100;
    private static readonly string? DefaultApiKey = Environment.GetEnvironmentVariable("API_KEY")
        ?? "sk-xxx";
    ...
    public static async Task<string> AdoGetSchemas(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return JsonConvert.SerializeObject(schemas, settings);
        }
        catch (Exception ex)
        {
    ...
    public static async Task<string> AdoGetTables(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return JsonConvert.SerializeObject(tables, settings);
        }
        catch (Exception ex)
        {
    ...
    public static async Task<string> AdoDescribeTable(
        [Description("Schema name")] string? schema = null,
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return JsonConvert.SerializeObject(tableInfo, settings);
        }
        catch (Exception ex)
        {
            throw new Exception($"Failed to describe table: {ex.Message}", ex);
        }
    }

    [McpServerTool(Name = "ado_filter_table_names"),
     Description("List tables whose names contain the given substring.")]
    public static async Task<string> AdoFilterTableNames(
    ...
    public static async Task<string> AdoExecuteQuery(
    ...
    public static async Task<string> AdoExecuteQueryMd(
        [Description("SQL query")] string query,
        [Description("Max Rows")] int? max_rows = null,
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    ...
    [McpServerTool(Name = "ado_query_database"),
     Description("Execute a SQL query and return results in JSONL format.")]
    public static async Task<string> AdoQueryDatabase(
        [Description("SQL query")] string query,
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    {
    ...
    public static async Task<string> AdoSpasqlQuery(
        ...
        CancellationToken cancellationToken = default)
    {
        var maxRowsValue = max_rows ?? 20;
        var timeoutValue = timeout ?? 300000;

        await using var conn = GetConnection(url);
        await conn.OpenAsync(cancellationToken);

        string cmdText = $"select Demo.demo.execute_spasql_query('{EscapeSql(query)}', ?, ?) as result";

        await using var cmd = (OdbcCommand)conn.CreateCommand();
        cmd.CommandText = cmdText;
        cmd.Parameters.AddWithValue("@1", maxRowsValue);
        cmd.Parameters.AddWithValue("@2", timeoutValue);
    ...
    public static async Task<string> AdoSparqlQuery(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        string cmdText = $"select \"UB\".dba.\"sparqlQuery\"('{EscapeSql(query)}', ?, ?) as result";

        await using var cmd = (OdbcCommand)conn.CreateCommand();
        cmd.CommandText = cmdText;
        cmd.Parameters.AddWithValue("@1", formatValue);
        cmd.Parameters.AddWithValue("@2", timeoutValue);
    ...
    public static async Task<string> AdoVirtuosoSupportAi(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        const string cmdText = "select DEMO.DBA.OAI_VIRTUOSO_SUPPORT_AI(?, ?) as result";
        await using var cmd = (OdbcCommand)conn.CreateCommand();
        cmd.CommandText = cmdText;
    ...
    public static async Task<string> AdoSparqlFunc(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        const string cmdText = "select DEMO.DBA.OAI_SPARQL_FUNC(?, ?) as result";
        await using var cmd = (OdbcCommand)conn.CreateCommand();
        cmd.CommandText = cmdText;
        cmd.Parameters.AddWithValue("@prompt", prompt);
        cmd.Parameters.AddWithValue("@apiKey", key);
    ...
    public static async Task<string> AdoSparqlGetEntityTypes(
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    {
  ...
  LIMIT 100
    ...
    [McpServerTool(Name = "ado_sparql_get_entity_types_detailed"),
     Description(@"This query retrieves all entity types in the RDF graph, along with their labels and comments if available. "
                 + "It filters out blank nodes and ensures that only IRI types are returned. "
                 + "The LIMIT clause is set to 100 to restrict the number of entity types returned.")]
    public static async Task<string> AdoSparqlGetEntityTypesDetailed(
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    {
    ...
    public static async Task<string> AdoSparqlGetEntityTypesSamples(
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    {
        var query = @"SELECT * FROM (
    ...
    public static async Task<string> AdoSparqlGetOntologies(
        [Description("ADO URL")] string? url = null,
        CancellationToken cancellationToken = default)
    {
        var query = @"SELECT * FROM (
